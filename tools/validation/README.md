...
No supported Markdown linter dependency or configuration exists, so CI runs the deterministic repository-defined Markdown hygiene checks rather than adding a separate toolchain. VAL-GOV-001 implementation is complete, but no repository-owned secret scanner exists. The gap is accepted and deferred for v0.4, subject to formal Security Review disposition before release; the external commit hook remains supplemental evidence and is not repository CI enforcement.

Sprint 4.4 implementation and governance closeout evidence are recorded in [SPR-004-004](../../product/sprints/SPR-004-004-governance-validation.md). Architecture, Domain, and Documentation decisions are recorded there; Security Review remains pending. Human governance gates are not satisfied by validator or CI success.
